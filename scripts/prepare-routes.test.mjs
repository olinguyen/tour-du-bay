...
import { prepareRoutes } from './prepare-routes.mjs';
...
/** the published module, in the same shape the script writes (its literal is JSON) */
function serialize(routes) {
  return `// Test fixture.\nexport const ROUTES: Record<string, GeneratedRoute> = ${JSON.stringify(routes)};\n`;
}

function parse(text) {
  const from = text.indexOf('export const ROUTES');
  return JSON.parse(text.slice(text.indexOf('= ', from) + 2).trim().replace(/;$/, ''));
}
...
async function fixture(t) {
  ...
  const saveData = () => fs.writeFile(destination, serialize(publish(plans, segments)));
  ...
  return {
    root, destination, checkpoint, plans, segments, savePlans, saveData, saveCatalog,
...
test('the checked-in route file validates against the plan and catalog without writes or fetches', async () => {
  const destination = new URL('src/data/routes.generated.ts', root);
  ...
  assert.match(before, /^\/\/ GENERATED by scripts\/prepare-routes\.mjs — do not edit\n/);
...
test('points are published as [lat, lng, whole feet] from the router’s [lon, lat, metres]', async t => {
  ...
  assert.match(text, /^\/\/ GENERATED by scripts\/prepare-routes\.mjs — do not edit\n/);
  assert.match(text, /\nexport const ROUTES: Record<string, GeneratedRoute> = \{\n/);
  const published = parse(text)['test-ride'];
  ...
  for (const target of ['checkpoint', 'published']) {
    for (const operation of ['write', 'rename']) {
      await t.test(`${target} ${operation}`, async t => {
        ...
        const matches = path => path.href.includes(target === 'checkpoint' ? '.route-data-checkpoint.json' : 'routes.generated.ts.');
        const files = {
          ...fs,
...
test('unreadable published data is never treated as a missing file', async t => {
  const f = await fixture(t);
  for (const text of ['garbage', 'export const ROUTES: X = {oops};\n', serialize({ 'test-ride': { source: {}, points: [] } })]) {
...
test('published parts require their saved routing inputs and consistent source attribution', async t => {
  ...
  for (const [name, mutate, error] of [
    ...
    await t.test(`published ${name}`, async t => {
      ...
      await fs.writeFile(f.destination, serialize(routes));
      await assert.rejects(f.run(), error);
    });
  }
});
